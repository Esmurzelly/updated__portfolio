...
const Projects = () => {
  ...
  return (
    <section id="projects" className="block_link w-full text-center flex flex-col items-center justify-center md:pt-20 py-8">
      <h1 className="w-[200px] font-bold uppercase border-solid border-4 border-black font-montserrat text-lg py-2 mt-3">
        projects
      </h1>

      <div className="w-full mt-8 bg-darkest_layout py-6 px-4 text-white grid grid-cols-2 grid-rows-2 gap-y-3">
        <button className="sort_button col-span-1" onClick={() => sortByType('web_app')}>app</button>
        <button className="sort_button col-span-2" onClick={() => sortByType('web_site')}>sites</button>
        <button className="sort_button col-span-3" onClick={() => sortByType('utility')}>utils</button>
      </div>
